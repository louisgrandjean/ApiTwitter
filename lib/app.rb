...
Dotenv.load
require 'twitter'#Apelle la gem Twitter
  ...
  def tweets_5_journalists
  	...
  	i = 0
  	while i < 5
        definitif = journalist.sample(1)
        client.update("#{definitif.join(' ')} #bonjour_monde petit message pour vous remercier de votre investissement @the_hacking_pro")
        i += 1
        sleep(2)
    end   
    end 
  ...
  def follow_user
    client = login
    client.search("#bonjour_monde", result_type: 'recent').take(22).each do |tweet|
